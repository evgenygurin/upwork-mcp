...
Uses the edit buttons on your profile page (requires browser via connect-chrome.bat).`,
    inputSchema: {
      ...
      properties: {
        ...
        hourly_rate: { type: ['number', 'string'], description: 'Hourly rate in USD' },
...
Use this to find potential clients and opportunities.`,
    inputSchema: {
      ...
      properties: {
        ...
        budget_min: { type: ['number', 'string'], description: 'Minimum budget in USD' },
        budget_max: { type: ['number', 'string'], description: 'Maximum budget in USD' },
        posted_within_days: { type: ['number', 'string'], description: 'Only jobs posted within N days. Default: 7' },
        limit: { type: ['number', 'string'], description: 'Max results. Default: 10' },
      },
      required: ['query'],
    },
  },
...
The agent will automatically type and submit the proposal.`,
    inputSchema: {
      ...
      properties: {
        ...
        bid_rate: { type: ['number', 'string'], description: 'Hourly rate or fixed price in USD' },
        screening_answers: {
          type: 'array',
          items: { type: 'string' },
